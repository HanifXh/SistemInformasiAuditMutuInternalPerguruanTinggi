...
<?php
    ...
    $query = mysqli_query($connect, "SELECT * FROM admin WHERE username='$username_session'");
...
<head>
  ...
  <title>Audit Mutu STMIK Bandung | <?php echo $rows['nama']; ?></title>
        ...
        <ul class="nav navbar-nav">   
          ...
          <li class="dropdown user user-menu">
            <a href="#" class="dropdown-toggle" data-toggle="dropdown">
              <img src="../images/<?php echo $rows['foto']; ?>" class="user-image" alt="User Image">
              <span class="hidden-xs"><?php echo $rows['nama']; ?></span>
            </a>
            <ul class="dropdown-menu">
              <!-- User image -->
              <li class="user-header">
                <img src="../images/<?php echo $rows['foto']; ?>" class="img-circle" alt="User Image">
  ...
  <!--     <form action="#" method="get" class="sidebar-form">
    ...
    <li class="treeview">
          ...
          <ul class="treeview-menu">
            ...
            <li><a href="index.php?page=cetak_instrumen_audit"><i class="glyphicon glyphicon-list"></i>Cetak Instrumen Audit</a></li>
        ...
        <li class="treeview">
          ...
          <ul class="treeview-menu">
            <li><a href="index.php?page=standar_operasional"><i class="glyphicon glyphicon-tags"></i>Standar</a></li>
        ...
        <li class="treeview">
          ...
          <ul class="treeview-menu">
             <li><a href="index.php?page=hasil_rata_rata"><i class="glyphicon glyphicon-save"></i>Hasil Rata-Rata</a></li>
              <li><a href="index.php?page=upmi_laporan"><i class="glyphicon glyphicon-user"></i>Hasil Audit</a></li>
     ...
     <!--  <li class="treeview">
        ...
        <li class="treeview">
          <a href="#">
            <i class="glyphicon glyphicon-tags"></i>
            <span>Standar STMIK Bandung</span>
            <span class="pull-right-container">
              <i class="fa fa-angle-left pull-right"></i>
            </span>
          </a>
         ...
         <li class="treeview">
          <a href="#">
            ...
            <span>Butir Standar</span>
            <span class="pull-right-container">
              <i class="fa fa-angle-left pull-right"></i>
            </span>
          </a>
          <ul class="treeview-menu">
            <li><a href="index.php?page=butir_sop"><i class="glyphicon glyphicon-file"></i>Butir Standar</a></li>
         ...
         <li class="treeview">
          ...
          <ul class="treeview-menu">
            <li><a href="index.php?page=dokumen"><i class="glyphicon glyphicon-file"></i>Upload Dokumen</a></li>
  ...
  if(isset($_GET['page'])){
    ...
    switch ($page) {
      ...
      case 'olah_akun_staf':
        ...
        break;
      case 'cetak_instrumen_audit':
        include "pages/butir/cetak_instrumen_audit.php";
        break;
      case 'hasil_rata_rata':
        include "pages/evaluasi/upmi_rata_rata.php";
        break;
      case 'upmi_laporan':
        include "pages/evaluasi/upmi_laporan.php";
        break;
 ...
 $(document).ready(function() {
    ...
    $('#example').DataTable( {
        ...
        buttons: [
            ...
            {
                extend: 'pdf',
                messageBottom:' Keterangan :1. Hasil diisi dengan mengisi symbol “V” di salah satu kolom 2. Persentase hasil diisi dengan mengisi persentase capaian berdasarkan indikator capaian yang telah ditentukan 3. Kategor temuan dapat diisi dengan [1] Ringan [2] Sedang dan [3] Berat'
...
<script>
  $(document).ready(function() {
        $('#konfirmasi_hapus_jadwal').on('show.bs.modal', function(e) {
            $(this).find('.btn-ok').attr('href', $(e.relatedTarget).data('href'));
        });
    });
</script>
...
<script>
  $(document).ready(function(){
    $('#edit_modal_jadwal').on('show.bs.modal', function(e){
      var idx = $(e.relatedTarget).data('id')
      //menggunakan fungsi ajax untuk mengambil data
    $.ajax({
      type :'post',
      url  : 'pages/jadwal/edit_jadwal.php',
      data : 'idx='+ idx,
      success : function(data){
        $('.hasil-data').html(data); //menampilkan data ke dalam modal
      console.log(idx);
...
<script>
  $(document).ready(function(){
    $('#edit_modal_download_dokumen').on('show.bs.modal', function(e){
      var idx = $(e.relatedTarget).data('id')
      //menggunakan fungsi ajax untuk mengambil data
    $.ajax({
      type :'post',
      url  : 'pages/butir/modal_download_dokumen.php',
      data : 'idx='+ idx,
      success : function(data){
        $('.hasil-data').html(data); //menampilkan data ke dalam modal
      console.log(idx);
...
<!-- VALIDASI FORM AUDITOR -->

<script>
  
  function validasidata_auditor(){

    var nama_auditor = document.getElementById('nama_auditor');
    var alamat_auditor = document.getElementById('alamat_auditor');
    var email_auditor = document.getElementById('email_auditor');
    var no_telp_auditor = document.getElementById('no_telp_auditor');
    var jabatan = document.getElementById('jabatan');



    if (harusDiisi(nama_auditor, "Nama Auditor Belum Diisi")){
      if (harusDiisi(alamat_auditor, "Alamat Auditor Belum Diisi")){
       if (harusDiisi(email_auditor, "Email Auditor Belum Diisi")){
        if (harusDiisi(no_telp_auditor, "No Telp Auditor Belum Diisi")){
         if (harusDiisi(jabatan, "Jabatan Auditor Belum Diisi")){
...
</script> <!-- PENUTUP VALIDASI DATA AUDITOR -->


<!-- VALIDASI DATA WAKIL KETUA -->

<script>
  
  function validasidata_wakil_ketua(){

    var nip_wakil_ketua = document.getElementById('nip_wakil_ketua');
    var nama_wakil_ketua = document.getElementById('nama_wakil_ketua');
    var email_wakil_ketua = document.getElementById('email_wakil_ketua');
    var no_telp_wakil_ketua = document.getElementById('no_telp_wakil_ketua');
    var jabatan_wakil_ketua = document.getElementById('jabatan_wakil_ketua');



    if (waketu(nip_wakil_ketua, "Nip Wakil Ketua Belum Diisi")){
      if (waketu(nama_wakil_ketua, "Nama Wakil Ketua Belum Diisi")){
       if (waketu(email_wakil_ketua, "Email Wakil Ketua Belum Diisi")){
        if (waketu(no_telp_wakil_ketua, "No Telp Wakil Ketua Belum Diisi")){
         if (waketu(jabatan_wakil_ketua, "Jabatan Wakil Ketua Belum Diisi")){
...
</script> <!-- PENUTUP VALIDASI DATA WAKIL KETUA -->


<!-- VALIDASI TAMBAH DATA DESKRIPTOR -->

<script>
  
  function validasidata_tambah_deskriptor(){

    var deskripsi = document.getElementById('deskripsi');
    var pengendali_dokumen = document.getElementById('pengendali_dokumen');
    

    if (deskriptor(deskripsi, "Deskriptor Belum Diisi")){
      if (deskriptor(pengendali_dokumen, "Pengendali Dokumen  Belum Diisi")){
...
</script> <!-- PENUTUP VALIDASI TAMBAH DATA DESKRIPTOR -->



<!-- VALIDASI FORM JADWAL AUDIT -->
      ...
      if (jadwal_audit(id_auditor, "Auditor Belum Diisi")){
       if (jadwal_audit(program_studi, "Program Studi Belum Diisi")){
        if (jadwal_audit(tahun_pengukuran, "Tahun Pengukuran Belum Diisi")){
         if (jadwal_audit(tanggal_audit, "Tanggal Audit Belum Diisi")){
...
</script> <!-- PENUTUP VALIDASI FORM JADWAL AUDIT -->



<!-- VALIDASI TAMBAH STANDAR NASIONAL -->

<script>
  
  function validasidata_tambah_sn(){

    var kode_sn = document.getElementById('kode_sn');
    var nama_sn = document.getElementById('nama_sn');
    var tanggal_sn = document.getElementById('tanggal_sn');
    



    if (tambah_sn(kode_sn, "Kode Standar Belum Diisi")){
      if (tambah_sn(nama_sn, "Nama Standar Belum Diisi")){
       if (tambah_sn(tanggal_sn, "Tanggal Standar Studi Belum Diisi")){
...
</script> <!-- PENUTUP VALIDASI TAMBAH STANDAR NASIONAL -->


<!-- VALIDASI TAMBAH SUB STANDAR -->

<script>
  
  function validasidata_tambah_sub_standar(){

    var kode_unit = document.getElementById('kode_unit');
    var kode_sn = document.getElementById('kode_sn');
    var kode_sop = document.getElementById('kode_sop');
    var nama_sop = document.getElementById('nama_sop');
    var id_penanggung_jawab = document.getElementById('id_penanggung_jawab');
    var tgl_sop = document.getElementById('tgl_sop');
    



    if (tambah_sub_standar(kode_unit, "Kode Unit Belum Diisi")){
      if (tambah_sub_standar(kode_sn, "Kode Standar Nasional Belum Diisi")){
       if (tambah_sub_standar(kode_sop, "Kode Sub Standar Belum Diisi")){
        if (tambah_sub_standar(nama_sop, "Nama Sub Standar Belum Diisi")){
         if (tambah_sub_standar(id_penanggung_jawab, "Penanggung jawab Belum Diisi")){
          if (tambah_sub_standar(tgl_sop, "Tanggal Sub Standar Belum Diisi")){
  ...
  function tambah_sub_standar(att, msg){
    if (att.value.length == 0){
      alert(msg);
      att.focus();
      return false;
...
</script> <!-- PENUTUP VALIDASI TAMBAH SUB STANDAR -->
